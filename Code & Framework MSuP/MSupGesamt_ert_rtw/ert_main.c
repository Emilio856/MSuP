...
void rt_OneStep(void)
{
   ...
   */
  MSupGesamt_SetEventsForThisBaseStep(eventFlags);
...
int main(void)
{
  ...
  ;
  rtmSetErrorStatus(MSupGesamt_M, 0);
  MSupGesamt_initialize();
  configureTimer0(modelBaseRate, systemClock);
  runModel =
    rtmGetErrorStatus(MSupGesamt_M) == (NULL);
  enableTimer0Interrupt();
  enable_interrupts();
